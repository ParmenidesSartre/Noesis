    ...
    intercept(context, next) {
        ...
        return next.handle().pipe((0, operators_1.tap)({
            next: () => {
                ...
                this.logger.http(`${request.method} ${request.url} - ${response.statusCode} - ${duration}ms`, {
                    correlationId,
            ...
            error: (error) => {
                const duration = Date.now() - startTime;
                this.logger.error(`${request.method} ${request.url} - Error: ${error.message} - ${duration}ms`, error.stack);
            },
        }));
    }
};
